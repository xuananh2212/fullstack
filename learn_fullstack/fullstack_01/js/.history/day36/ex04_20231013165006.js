...
const getProfile = async () => {
  ...
  console.log(nameEl);
  nameEl.innerHTML = data.name + data.role + data.avatar;
};
...
<ul class="profile list-unstyled d-flex gap-2"><li>chao ban <b><span class="name"></span></b></li> <li><a href="#" class="logout">dang xuat</a></li></ul>
  ...
  if (localStorage.getItem("access_token")) {
    ...
    logInForm.addEventListener("submit", (e) => {
      ...
      const email1 = emailEl.value;
      const password = passwdEl.value;
      handleLogin({ email1, password });
    });
  }
};
render();
...
const handleLogin = async (data) => {
  const { response, data: tokens } = await client.post("/auth/login", data);
  const { access_token, refresh_token } = tokens;
  if (access_token && refresh_token) {
    localStorage.setItem("access_token", access_token);
    localStorage.setItem("refresh_token", refresh_token);
  } else {
    console.log("đăng nhaajp thất bại!");
  }
