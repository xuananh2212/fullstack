...
const all = alldomail.map((emailItem) => {
...
// non -canonical Group: ?: để loại bỏ cannoical.
const url = "https://fullstack.edu.vn/khoa-hoc/fullstack-offline";
const pattern3 = /(?:http|https):\/\/[a-z-_0-9\.]+\.([a-z]{2,})\/.*/;
const result = url.match(pattern3);
console.log(result);

//greedy là viết dấu ? độ dài giỏi là greedy.

const img = `<img width="100px" height="100" src="https://fullstack.edu.vn" alt=""  height="100" height="100">`;

const pattern4 = /img.*src="(.+?)"/;

console.log(img.match(pattern4));

// thay thế. không nên sử dụng replaceAll thay về đó dùng replace và biểu thức chính quy thêm /..../g

var content1 = `số điện thoại của tôi là 0338882822 . thêm số nữa 0981081928`;
const pattern5 = /(0|\+84)\d{9}/g;
content1 = content1.replace(pattern5, "**");
console.log(content1);

// đổi sách chuỗi chỉ hoạt động với capturing group bỏ dấu ().

var content1 = `số điện thoại của tôi là 0338882822 . thêm số nữa 0981081928`;

const pattern6 = /((0|\+84)\d{9})/g;

content1 = content1.replace(pattern6, `<a href="tel:$1">$1</a>`);
console.log(content1);

const a = `abc\/\d+`;
// chuyển đổi thành pattern.

const pattern2 = new RegExp(a, "g");
